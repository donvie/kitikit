            ...
            <q-card square bordered>
              <q-img style="height: 150px;" :src="product.images[0]">
                <div style="padding: 5px;" class="absolute-top-right bg-transparent">
                  <q-btn icon="message" @click="viewChat(product)" round dense unelevated color="brown" />
                </div>
      ...
      <q-layout view="Lhh lpR fff" container class="bg-white">
        <q-header class="bg-brown">
          <q-toolbar>
            ...
            <q-toolbar-title v-if="user">Chat</q-toolbar-title>
          </q-toolbar>
        </q-header>

        <q-footer class="bg-white">
          <q-input class="q-ma-xs" v-model="message" autogrow borderless filled placeholder="Enter message">
...
export default {
  ...
  mounted () {
    this.loadData()
    if (this.$q.localStorage.getItem('productData')) {
      this.$binding('messagesFromTo', this.firebase.firestore().collection('messages').orderBy('stamp', 'desc').where('from', '==', this.user.uid).where('to', '==', this.$q.localStorage.getItem('productData').uid))
      this.$binding('messagesToFrom', this.firebase.firestore().collection('messages').orderBy('stamp', 'desc').where('to', '==', this.user.uid).where('from', '==', this.$q.localStorage.getItem('productData').uid))
    }
  },
  computed: {
    filteredMessage () {
      let msg = this.messagesFromTo.concat(this.messagesToFrom).sort((a, b) => {
  ...
  methods: {
    ...
    viewChat (product) {
      if (product.uid !== this.user.uid) {
        this.$q.localStorage.set('productData', product)
        if (this.$q.localStorage.getItem('productData')) {
          this.$binding('messagesFromTo', this.firebase.firestore().collection('messages').orderBy('stamp', 'desc').where('from', '==', this.user.uid).where('to', '==', this.$q.localStorage.getItem('productData').uid))
          this.$binding('messagesToFrom', this.firebase.firestore().collection('messages').orderBy('stamp', 'desc').where('to', '==', this.user.uid).where('from', '==', this.$q.localStorage.getItem('productData').uid))
        }
        ...
        this.$q.notify({
          icon: 'close',
          message: 'You can not message this product.',
          timeout: 800,
          color: 'negative',
          textColor: 'white',
